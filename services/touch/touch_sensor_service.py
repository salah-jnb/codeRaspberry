...
class TouchSensorService:
    ...
    def start(self, on_touch: Callable[[], None]) -> bool:
        ...
        logger.warning(
            "Touch sensor unavailable on GPIO%d. Install gpiozero or RPi.GPIO on the Pi.",
            self._pin,
        )
        return False
    ...
    def _start_gpiozero(self, emit_touch: Callable[[], None]) -> bool:
        ...
        try:
            ...
            device.when_activated = emit_touch
            self._device = device
            logger.info(
                "Touch sensor ready on GPIO%d via gpiozero (active_high=%s, pull_up=%s, cooldown=%.2fs)",
                self._pin, self._active_high, self._pull_up, self._cooldown,
            )
            return True
        except Exception:
    ...
    def _start_rpi_gpio(self, emit_touch: Callable[[], None]) -> bool:
        ...
        try:
            ...
            GPIO.add_event_detect(
                self._pin,
                edge,
                callback=lambda _pin: emit_touch(),
                bouncetime=max(1, int(self._bounce * 1000)),
            )
            self._gpio = GPIO
            logger.info(
                "Touch sensor ready on GPIO%d via RPi.GPIO (active_high=%s, pull_up=%s, cooldown=%.2fs)",
                self._pin, self._active_high, self._pull_up, self._cooldown,
            )
            return True
        except Exception:
